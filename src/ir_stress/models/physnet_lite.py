"""PhysNet-lite — reduced-width 3D CNN backbone for lower memory use.
Comparison with PhysNet:
- 32 channels vs 64 channels
- 1 conv block per stage vs 2 conv blocks per stage
"""
...
def _conv_block(ch: int) -> nn.Sequential:
    return nn.Sequential(
        nn.Conv3d(ch, ch, kernel_size=(3, 3, 3), stride=1, padding=(1, 1, 1)),
        nn.BatchNorm3d(ch),
        nn.ELU(),
    )


class PhysNetLite(Backbone):
    """Lightweight PhysNet variant: 32 channels, one conv block per stage."""

    def __init__(self, in_ch: int = 1, grad_checkpoint: bool = False):
        super().__init__()
        self.grad_checkpoint = grad_checkpoint
        self.start = nn.Sequential(
            nn.Conv3d(in_ch, 16, kernel_size=(1, 5, 5), stride=1, padding=(0, 2, 2)),
            nn.BatchNorm3d(16),
            nn.ELU(),
        )
